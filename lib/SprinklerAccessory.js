...
class SprinklerAccessory extends BaseAccessory {
    ...
    _registerCharacteristics(dps) {
        ...
        this.defaultDurationSecs = this.device.context.defaultDurationSecs
        ...
        this.device.on('change', changes => {
            ...
            if (changes.hasOwnProperty(this.dpPower) && characteristicInUse.value !== changes[this.dpPower]) characteristicInUse.updateValue(changes[this.dpPower]);
            
            if (!this.noTimer) {
                if (changes.hasOwnProperty(this.dpPower) && !changes[this.dpPower]){
                    clearTimeout(service.timer);
                    characteristicRemainingDuration.updateValue(0);
                }
            }
        });
    }
