...
class RGBTWAccessory extends BaseAccessory {
    ...
    _instrument() {
        ...
        const service = this.accessory.addService(Service.Lightbulb, context.name);
    ...
    getBrightness(callback) {
        ...
        this.getState(['2', '3', '5'], (err, value) => {
            ...
            if (value['2'] === 'white') return callback(null, this.getBrightnessFromTuya(value['3']));
            callback(null, this.getColorFromTuya(value['5']).b)
        });
    }

    setBrightness(newValue, callback) {
        this.getState(['2', '3', '5'], (err, value) => {
            if (err) return callback(err);
            if (value['2'] === 'white') return this.setState('3', this.getBrightnessForTuya(newValue), callback);
            this.setState('5', this.getColorForTuya({b: newValue}), callback);
        });
    }

    getColorTemperature(callback) {
        this.log('getColorTemperature');
        this.getState('4', (err, value) => {
            if (err) return callback(err);
            const newValue = this.getColorTemperatureFromTuya(value);
    ...
    setColorTemperature(value, callback) {
        ...
        async.series([
            (next) => {
                this.setState('2', 'white', next);
            },
            (next) => {
                this.setState('4', newValue, next);
            }
        ], callback);
    }
    ...
    getHue(callback) {
        ...
        this.getState('5', (err, value) => {
            if (err) return callback(err);
            const newValue = this.getColorFromTuya(value).h;
            callback(null, newValue);
        });
    }
    ...
    getSaturation(callback) {
        this.log('getSaturation');
        this.getState('5', (err, value) => {
            if (err) return callback(err);
            const newValue = this.getColorFromTuya(value).s;
            callback(null, newValue);
        });
    }
    ...
    _setHueSaturation(prop, callback) {
        ...
        if (prop) {
            ...
            this._pendingHueSaturation.props = {...this._pendingHueSaturation, ...prop};
        ...
        async.series([
            (next) => {
                this.setState('2', 'colour', next);
            },
            (next) => {
                this.setState('5', newValue, next);
            }
        ], err => {
