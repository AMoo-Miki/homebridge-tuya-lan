class BaseAccessory {
    ...
    convertColorFromHomeKitToTuya_HEXHSB(value, dpValue) {
        const cached = this.convertColorFromTuya_HEXHSB_ToHomeKit(dpValue || this.device.state['5']);
    ...
    convertColorFromHomeKitToTuya_HSB(value, dpValue) {
        const cached = this.convertColorFromTuya_HSB_ToHomeKit(dpValue || this.device.state['5']);
        let {h, s, b} = {...cached, ...value};
