class BaseAccessory {
    ...
    _update(callback) {
        ...
        this.tuya.get({schema: true})
            .then(schema => {
                self.log.debug('Got schema', schema.dps);
    ...
    getState(dp, callback) {
        this.log.debug('Getting status for', dp);
        const self = this;
        const _callback = () => {
            if (Array.isArray(dp)) {
                const ret = {};
                ...
                self.log.debug('Got status', dp, 'for', ret);
                callback(null, ret);
            } else {
                self.log.debug('Got status', dp, 'for', self._cache.dps[dp]);
                callback(null, self._cache.dps[dp]);
            }
        };

        if (Date.now() - self._cache.time < 1000) return process.nextTick(_callback);
        this._update(_callback);
    }

    setState(dp, value, callback) {
        this.log.debug('Setting status for', dp, value);
        const self = this;
