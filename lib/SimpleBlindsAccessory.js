...
class SimpleBlindsAccessory extends BaseAccessory {
    ...
    constructor(...props) {
        ...
        this.duration = parseInt(this.device.context.timeToOpen) || 15;
    }
    ...
    _registerCharacteristics(dps) {
        ...
        this.assumedPosition = this._getCurrentPosition(dps['1']);
            ...
            if (changes.hasOwnProperty('1')) {
                switch (changes['1']) {
                    ...
                    case this.cmdClose:  // Starting to close
                        ...
                        if (this.targetPosition === false || this.targetPosition === BLINDS_CLOSED) {
                            ...
                            this.changeTimeout = setTimeout(() => {
                                ...
                                characteristicPositionState.updateValue(Characteristic.PositionState.STOPPED);
                                this.changeTime = false;
                                this.assumedPosition = BLINDS_CLOSED;
                                this.assumedState = BLINDS_STOPPED;
                    ...
                    case '3':  // Starting to close
                        ...
                        this.assumedPosition = Math.min(100, Math.max(0, Math.round((Date.now() - this.changeTime) / (10 * this.duration))));
                        ...
                        characteristicPositionState.updateValue(Characteristic.PositionState.STOPPED);
                        console.log("[TuyaAccessory] Blinds marked stopped at " + this.assumedPosition);
    ...
    setTargetPosition(value, callback) {
        console.log('[TuyaAccessory] Blinds asked to move to ' + value + ' from ' + this.assumedPosition);

        if (this.changeTimeout) clearTimeout(this.changeTimeout);
        this.targetPosition = value;

        if (this.changeTime !== false) {
            const assumedPosition = (Date.now() - this.changeTime) / (10 * this.duration);
            if (assumedPosition < 100) this.assumedPosition = assumedPosition;
        }
        console.log("[TuyaAccessory] Blinds assumedPosition is " + this.assumedPosition);
        const duration = Math.abs(this.assumedPosition - value) * this.duration * 10;

        if (value === this.assumedPosition) {
            return this.setState('1', '3', callback);
        } else if (value > this.assumedPosition) {
            this.setState('1', this.cmdOpen, callback);
            this.changeTime = Date.now() -  Math.abs(this.assumedPosition - BLINDS_CLOSED) * this.duration * 10;
        } else {
