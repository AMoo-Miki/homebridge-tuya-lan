...
class TuyaAccessory extends EventEmitter {
    constructor(props) {
        ...
        this._msgQueue = async.queue(this[this.context.version < 3.3 ? '_msgHandler_3_1': '_msgHandler_3_3'].bind(this), 1);
    ...
    _connect() {
        ...
        this._socket.on('error', err => {
            this.connected = false;
            console.log(`[TuyaAccessory] Socket had a problem and will reconnect to ${this.context.name} (${err && err.code || err})${this.context.version === '3.3' ? '. This is common for v3.3 devices.' : ''}`);
    ...
    _msgHandler_3_1(task, callback) {
        ...
        switch (cmd) {
            ...
            case 8:
                ...
                let decryptedMsg = decipher.update(data.substr(19), 'base64', 'utf8');
                ...
                try {
                    ...
                    console.log(`[TuyaAccessory] Raw message from ${this.context.name} with command ${cmd}:`, task.msg.toString('hex'));
                    break;
                }
            ...
            case 10:
                if (data) {
                    try {
                        ...
                        console.log(`[TuyaAccessory] Raw update from ${this.context.name} with command ${cmd}:`, task.msg.toString('hex'));
                        break;
                    }
    ...
    _msgHandler_3_3(task, callback) {
        ...
        const versionPos = task.msg.indexOf('3.3');
        ...
        if (cmd === 10 && decryptedMsg === 'json obj data unvalid') {
            console.log(`[TuyaAccessory] ${this.context.name} didn't respond with its current state.`);
            this.context.pingTimeout = 90;
            this.emit('change', {}, this.state);
            return callback();
        }
    ...
    _send(o) {
        ...
        if (this.context.version < 3.3) return this._send_3_1(o);
    ...
    _send_3_3(o) {
        ...
        const tail = [];
        //version
        if (cmd !== 9 && cmd !== 10) tail.push('332e33000000000000000000000000');
        //data
        if (data) {
            const cipher = crypto.createCipheriv('aes-128-ecb', this.context.key, '');
        ...
        //end-mask
        tail.push('000000000000aa55');

        const suffix = Buffer.from(tail.join(''), 'hex');

        const prefix = Buffer.from('000055aa00000000000000' + cmd.toString(16).padStart(2, '0'), 'hex');
        prefix.writeUInt32BE(this._sendCounter++, 4);
