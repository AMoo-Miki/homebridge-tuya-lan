...
class TuyaAccessory extends EventEmitter {
    constructor(props) {
        ...
        if (!(props.id && props.key && props.ip)) return console.log('TuyaAccessory doesn\'t have enough details to initialize.');
    ...
    _connect() {
        ...
        (this._socket.reconnect = () => {
            ...
            this._socket.setTimeout((this.context.connectTimeout || 30) * 1000, () => {
        ...
        this._socket.on('data', msg => {
            const result = TuyaAccessory.translate(msg);
            switch (result.cmd) {
                case 7:
                    // ignoring
                    break;
                ...
                case 8:
                    if (typeof result.data === 'string') {
                        ...
                        try {
                            ...
                            console.log('Odd message with command 8:', result);
                        }
                ...
                default:
                    console.log('Unhandled message with command', result.cmd + ':', result);
            }
        });
    ...
    static discover(options) {
        console.log('Tuya discovery starting');
        let opts = options || {};
        ...
        server.on('error', err => {
            console.log(`Tuya discovery error:\n${err.stack}`);
            server.close();
        });

        server.on('close', () => {
            console.log('Tuya discovery closed.', server._stopRequested ? '' : 'Restarting...');
        ...
        server.on('message', msg => {
            let match;
            if ((match = msg.toString('utf8').match(/"gwId"\s*:\s*"([0-9a-fA-F]+)"/)) === null || _discovery.discovered.has(match[1])) return;
            ...
            if (Array.isArray(_discovery.limitedIds) &&
                ...
                _discovery.limitedIds.indexOf(result.data.id) !== -1 &&
        ...
        _discovery.destroy = () => {
            if (!server) return;
            _discovery.emit('end', result.data);
            server._stopRequested = true;
        ...
        (_discovery.start = () => {
            if (!server) return TuyaAccessory.discover(opts);
            server.bind(6666);
        })();
