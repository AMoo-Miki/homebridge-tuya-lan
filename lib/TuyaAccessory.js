...
class TuyaAccessory extends EventEmitter {
    constructor(props) {
        ...
        if (!(props.id && props.key && props.ip)) return console.log('[TuyaAccessory] Insufficient details to initialize:', props);
    ...
    update(o) {
        ...
        if (hasDataPoint) {
            ...
            if (result) {
                //console.log('[TuyaAccessory] Override from', this.context.name, '***********************************************');
                //this._change(dps);
            }
        } else {
            result = this._send({
